...
function estimateEtaMinutes(distanceKm) {
  ...
  return (distanceKm / walkingKmh) * 60;
}
...
function updateEtaLabel(from, to) {
  ...
  etaLabel.textContent = `ETA: ${Math.round(eta)} min`;
}
...
function drawRoute(destLat, destLng) {
  ...
  routingControl.on('routesfound', (e) => {
    ...
    if (safetyNote) {
      safetyNote.textContent = `Route ready. Approx. ${Math.round(eta)} min walk • ${distanceKm.toFixed(1)} km`;
    }
